...
window.onload = function() {
  ...
  socket.on('render', function(players, hidingObjects){
    initObjectsAndPlayers(players, hidingObjects, ctx);
  });
...
function renderPlayers(players, ctx) {
  ...
  if(players.current.wonGame || players.other.wonGame)
  {
    ...
    if(players.current.id == "player" || players.other.id == "player")
    {
      ...
      // gameOver = true;
      // // Check if space was pressed to restart the game
      // window.onkeydown = function(event)
      // {
      //   if(event.keyCode == 32)
      //   {
    ...
    // Indicate if player 2 won by killing the player
    else
    {
